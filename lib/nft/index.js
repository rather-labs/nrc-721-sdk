...
module.exports = ({
  ...
  const TOKEN_HEADER = keccak256("NRC-721T").toString("hex");
  ...
  const parseData = (hexData) => {
    ...
    hexData = hexData.slice(2 + TOKEN_HEADER.length);
  ...
  const mint = async ({
    ...
    const hexData = "0x" + TOKEN_HEADER + dataToHex(data).slice(2);
    const outputIndex = 0;
  ...
  const _isCellNRC721 = (nftCellData) => {
    const hexData = nftCellData.slice(2, 2 + TOKEN_HEADER.length);
    return hexData === TOKEN_HEADER;
  };
