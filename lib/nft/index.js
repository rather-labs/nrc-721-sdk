...
module.exports = ({
  ...
  const createNewTypeScript = ({ rawTransaction, factoryTypeScript, nftContractTypeScript, outputIndex }) => {
    /*
      Args:
  ...
  const mint = async ({
    ...
    const rawTransaction = ckb.generateRawTransaction({
      ...
      safeMode: true,
      cells: inputCells,
      deps: [ckb.config.secp256k1Dep, nftTypeCell, factoryCellRaw],
    });

    // Add cell with nft
    const nftTypeScript = await createNewTypeScript({ rawTransaction, factoryTypeScript: factoryCellRaw.type, nftContractTypeScript, outputIndex });
  ...
  const read = async (nftCell_typeScript) => {
    ...
    return { data, factoryData, rawCell: nftCells.cells[0] };
  };
