...
module.exports = ({
  ...
  const dataToHex = (data, prefix = "0x") => {
    ...
    const hexData = prefix + dataBuffer.toString("hex");
  ...
  const mint = async ({
    ...
    const outputIndex = 0;
    const headerData = keccak256("NRC-721F").toString("hex");
    const hexData = "0x" + headerData + dataToHex(factoryCellData, "") + extraData.toString("hex");
