const { CKBToShannon, serializeInputCell, getCellOccupiedCapacity, hxShannonToCKB, getPaymentCells } = require("../utils/utils");
...
module.exports = ({
  ...
  const blake2b_8 = ckb.utils.blake2b(16, null, null, null);
  blake2b_8.update(Buffer.from("NRC-721F"));
  const headerHash = Buffer.alloc(16);
  blake2b_8.digest(headerHash);
  ...
  const update = async ({
    ...
    if ((capacityDifference > 0) && (capacityDifference > fee)) {
      ...
      // get unspent cells
      inputCells = (await getLiveCellsByLock({ lockScript: sourceLockScript })).cells;
