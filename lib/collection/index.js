...
module.exports = ({
  ...
  const createNewTypeScript = async ({ rawTransaction, outputIndex }) => {
    /*
      Args:
  ...
  const dataToHex = (data) => {
    ...
    stringbuffers.push(stringTobuffer(data.tokenUri, COLECCTION_CELL_DATA_SIZES.tokenUri_lenght));
  ...
  const parseData = (hexData) => {
    ...
    ({ string: data.tokenUri, offset } = bufferToString(dataBuffer, offset, COLECCTION_CELL_DATA_SIZES.tokenUri_lenght));
  ...
  const mint = async ({
    ...
    const outputIndex = 0;
    const hexData = dataToHex(collectionCellData);
    ...
    // Add Collection cell to tx output
    const collectionTypeScript = await createNewTypeScript({ rawTransaction, outputIndex });
    rawTransaction.outputs[outputIndex].type = collectionTypeScript;
    rawTransaction.outputsData[outputIndex] = hexData;

    return { rawTransaction, typeScript: collectionTypeScript, usedCapacity };
  };

  const readOne = async (typeScript) => {
    const collectionCells = await getLiveCellsByType({ typeScript });
    if (collectionCells.cells.length === 0) {
      throw new Error("Validation Error: no cell found for type script");
    } else if (collectionCells.cells.length > 1) {
      throw new Error("Validation Error: more than one cell for type script");
    }
    // parse cell data
    const collectionData = parseData(collectionCells.cells[0].data);

    collectionCells.cells[0].type = typeScript;

    return { data: collectionData, rawCell: collectionCells.cells[0] };
  };
